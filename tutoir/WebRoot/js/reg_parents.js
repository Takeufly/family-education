window.onload=function(){
	...
	oLogname.onchange=function(){
		ajax("LognameCheck?type=parent&logname="+this.value+"&t="+new Date().getTime(),function(str){
			if(str=="false"){
				alert("用户名已被注册");
				oLogname.value="";
			}
	...
	oTel.onkeydown=function(ev){
		var oEvent=ev||event;
		if((oEvent.keyCode<96 || oEvent.keyCode>105) && oEvent.keyCode!=8){
			return false;
		}
	}
	...
	oBtn.onclick=function(){
		...
		if(oAddress.value=="" || oAddress.value==null){
			alert("请填写地址");
			return false;
		}
		if(oTel.value=="" || oTel.value==null){
			alert("请填写联系方式");
			return false;
		}
